var Q = require('q')
  ...
  , fileFuncs = require('./file-functions')
;

function moduleExports(mod) {
  return Object.keys(mod).reduce(function(obj, key) {
    obj[key] = mod[key];
    return obj;
  }, {});
}
...
function Runner(file, target, vars, options) {
...
_.extend(Runner.prototype, {
  ...
  run: function() {
    ...
    return this._ruleDefinition().
      then(function(rules) {
        var builder = new Builder(rules, self.scriptGlobal, self.options);
  ...
  _initialGlobals: function() {
    return _.extend(moduleExports(fileFuncs),
    {
      drt: {
        include_path: ['.']
      },
